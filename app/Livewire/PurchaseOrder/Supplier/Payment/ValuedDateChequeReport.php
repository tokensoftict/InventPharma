...
class ValuedDateChequeReport extends Component
{
    use LivewireAlert;
    ...
    public function render()
    {
        ...
        switch ($this->date_filter) {
            case 'Today':
                $query->whereDate('payment_info->going_out_date', Carbon::today());
                break;
            case 'This Week':
                $query->whereBetween('payment_info->going_out_date', [Carbon::now()->startOfWeek()->format('Y-m-d'), Carbon::now()->endOfWeek()->format('Y-m-d')]);
                break;
            case 'This Month':
                $query->whereBetween('payment_info->going_out_date', [Carbon::now()->startOfMonth()->format('Y-m-d'), Carbon::now()->endOfMonth()->format('Y-m-d')]);
                break;
            case 'Custom':
                if ($this->start_date && $this->end_date) {
                    $query->whereBetween('payment_info->going_out_date', [
        ...
        $payments = $query->orderBy('payment_info->going_out_date', 'ASC')->get();
        ...
        $grouped_payments = $payments->groupBy(fn($payment) => $payment->payment_info['going_out_date'] ?? $payment->payment_date->format('Y-m-d'));

        return view('livewire.purchase-order.supplier.payment.valued-date-cheque-report', [
