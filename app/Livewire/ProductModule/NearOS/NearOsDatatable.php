...
final class NearOsDatatable extends PowerGridComponent
{
    ...
    */
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('current_qty')
            ->add('current_sold')
            ->add('qty_to_buy_1m', function(Nearoutofstock $nearoutofstock){
                return !is_null($nearoutofstock->qty_to_buy_1m) ?  $nearoutofstock->qty_to_buy_1m : "N/A";
            })
            ->add('highest_qty', function(Nearoutofstock $nearoutofstock){
                return  $nearoutofstock->stockgroup_id === NULL ? ($nearoutofstock->stock->highest_qty_sold ?? "N/A") : "N/A";
            })
            ->add('group_os_id')
            ->add('is_grouped')
            ->add('last_qty_purchased')
            ->add('last_purchase_date')
            ->add('last_purchase_date_fomartted', function(Nearoutofstock $nearoutofstock){
                return $nearoutofstock->last_purchase_date != NULL ?  (new Carbon($nearoutofstock->last_purchase_date))->format('d/m/Y') : "N/A";
            })
            ->add('purchaseitem_id');
    ...
    */
    ...
    public function actions(Nearoutofstock $row): array
    {
        return [
