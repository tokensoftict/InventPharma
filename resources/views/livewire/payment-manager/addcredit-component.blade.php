                ...
                <div class="mb-3">
                    ...
                    <select class="form-control form-control-lg" wire:model="online_credit_invoice">
                        ...
                        @foreach($this->pendingOnlineInvoices as $invoice)
                            <option data-amt="{{ $invoice->total_amount_paid }}"  value="{{ $invoice->id }}">Invoice #{{ $invoice->id }} - {{ money($invoice->total_amount_paid) }} - {{ $invoice->invoice_date->format("Y-m-d") }}</option>
                        @endforeach
                    </select>
                </div>
            @endif
            ...
            return {
                ...
                init()
                {
                    ...
                    window.addEventListener('openPaymentPage', (e) => {
                        window.location.href = e.detail[0].link
                    });
                }
            };
        }
